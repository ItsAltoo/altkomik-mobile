  ...
  <Link href={`/read/${item.slug}` as unknown as Href} asChild>
    <Pressable className="flex-row items-center justify-between rounded-lg border border-outline-50 bg-background-50 p-3.5 active:bg-background-100">
      <HStack className="items-center gap-3">
        ...
        <VStack>
          <Text className="text-[14px] font-bold text-typography-900">{item.title}</Text>
  ...
  return (
    <VStack className="mb-8 gap-4">
      ...
      <HStack className="items-center gap-2">
        ...
        <Button size="md" className="items-center justify-center bg-primary-500" onPress={toggleSort}>
          <Icon as={sortOrder === "desc" ? ArrowDownAZ : ArrowUpAZ} className="size-5 text-typography-white" />
        </Button>
      </HStack>
