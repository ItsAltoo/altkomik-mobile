...
import { Clock, Eye, Calendar, Tag } from 'lucide-react-native';
...
export const ComicCard = ({
  ...
  const isValidImage = Boolean(
    thumbnail && thumbnail.startsWith('http') && !thumbnail.startsWith('https://komiku.org')
  );

  return (
    <Box className={`flex-1 bg-background-0 border border-outline-100 shadow-soft-1 rounded-xl overflow-hidden ${className}`}>
      {/* IMAGE CONTAINER */}
      <Link href={apiDetailLink} asChild>
        <Pressable className="w-full h-[180px] relative overflow-hidden bg-background-50 active:scale-[0.98] transition-transform duration-200">
          ...
          {finalLatestChapter && (
            ...
            >
              <BadgeText className="text-[10px] font-bold text-typography-900" numberOfLines={1}>
                {finalLatestChapter}
              </BadgeText>
            </Badge>
          )}
      ...
      <VStack className="p-3 gap-3 flex-1 justify-between bg-background-0">
        <Link href={apiDetailLink} asChild>
          <Pressable className="active:opacity-70 transition-opacity">
            <Text
              numberOfLines={2}
              className="font-extrabold text-sm leading-[18px] text-typography-900 tracking-tight"
            >
              {title}
            </Text>
            {description && (
              <Text numberOfLines={2} className="text-[10px] leading-[14px] text-typography-500 mt-1">
                {description}
              </Text>
            )}
          </Pressable>
          ...
          <VStack className="gap-1.5">
            {timeAgo && (
              <HStack className="items-center gap-1.5">
                ...
                <Text className="text-[11px] font-medium text-typography-500" numberOfLines={1}>{timeAgo}</Text>
              </HStack>
            )}
            {views && (
              <HStack className="items-center gap-1.5">
                <Icon as={Eye} className="text-primary-500 w-3.5 h-3.5" />
                <Text className="text-[11px] text-typography-500" numberOfLines={1}>{views}</Text>
              </HStack>
            )}
            {release && (
              <HStack className="items-center gap-1.5">
                <Icon as={Calendar} className="text-primary-500 w-3.5 h-3.5" />
                <Text className="text-[11px] text-typography-500" numberOfLines={1}>{release}</Text>
              </HStack>
            )}
            {genre && (
              <HStack className="items-center gap-1.5">
                <Icon as={Tag} className="text-primary-500 w-3.5 h-3.5" />
                <Text className="text-[11px] text-typography-500" numberOfLines={1}>{genre}</Text>
              </HStack>
            )}
          </VStack>
          ...
          <HStack className="w-full gap-2">
            {apiInitialChapterLink && apiInitialChapterLink !== apiChapterLink && (
              <Link href={apiInitialChapterLink} asChild>
                <Button size="sm" variant="outline" className="flex-1 h-7 px-1 rounded-md border-outline-200">
                  <ButtonText className="text-[10px] font-bold text-typography-700">Read First</ButtonText>
                </Button>
              </Link>
            )}
            {apiChapterLink ? (
              <Link href={apiChapterLink} asChild>
                <Button size="sm" variant="solid" className="flex-1 h-7 px-1 rounded-md">
              ...
              <Link href={apiDetailLink} asChild>
                <Button size="sm" variant="solid" className="flex-1 h-7 px-1 rounded-md">
                  <ButtonText className="text-[10px] font-bold">Details</ButtonText>
                </Button>
              </Link>
            )}
          </HStack>
